#!/bin/bash

#for split in 10-90 20-80 30-70 40-60 50-50 60-40 70-30 80-20 90-10; do
#    for dtype in lisp fol; do
#        for model in small medium large huge; do
#            export SPLIT=${split}
#            sbatch slurm_scripts/text_to_lamp/${dtype}/decode_codegen_${model}.sh
#        done
#    done
#done

#for split in 50-50; do # do 50-50 across all models and data 
#    for dtype in fol lisp; do
#        for model in small medium large huge; do
#            export SPLIT=${split}
#            sbatch slurm_scripts/text_to_lamp/${dtype}/decode_codegen_${model}.sh
#        done
#    done
#done

#for split in 0-100 10-90 20-80 30-70 40-60 50-50 60-40 70-30 80-20 90-10 100-0; do # range across all splits 
for split in 0-100 10-90 20-80 30-70 40-60 60-40 70-30 80-20 90-10 100-0; do # range across all splits 
#for split in  40-60 60-40 70-30 80-20 90-10 100-0; do # range across all splits 
    for dtype in fol; do # for now, just do FOL 
        for model in large huge; do # for now, just do medium 
        #for model in small medium large huge; do # do all of them  
            export SPLIT=${split}
            sbatch slurm_scripts/text_to_lamp/${dtype}/decode_codegen_${model}.sh
        done
    done
done

