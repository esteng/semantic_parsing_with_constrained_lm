...
class MyCodeGenPreTrainedModel(PreTrainedModel):
    """
    ...
    def _set_gradient_checkpointing(self, module, value=False):
        if isinstance(module, MyCodeGenModel):
...
class MyCodeGenModel(MyCodeGenPreTrainedModel):
    def __init__(self, config):
...
class MyCodeGenForCausalLM(MyCodeGenPreTrainedModel):
    _keys_to_ignore_on_load_missing = [r"h\.\d+\.attn\.masked_bias", r"h\.\d+\.attn\.bias"]

    def __init__(self, config):
        super().__init__(config)
        self.transformer = MyCodeGenModel(config)
