...
from semantic_parsing_with_constrained_lm.paths import DOMAINS_DIR, BENCH_CLAMP_PROCESSED_DATA_DIR
...
def read_data(
    ...
    if dataset == "calflow" and output_type in [
        ...
        calflow_train_data_file = (
            DOMAINS_DIR / "calflow/data/train_full.jsonl"
        )
        calflow_eval_data_file = DOMAINS_DIR / "calflow/data/dev_full.jsonl"
        train_data = [
            datum
        ...
        eval_data = [
            datum
            for datum in read_calflow_jsonl(calflow_eval_data_file, output_type)[:1600]
            if datum.canonical is not None
        ]
